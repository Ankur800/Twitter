...
public class GenericDB<T> {
    ...
    public void deleteRow(TableImpl<?> table, String tableName, String columnName, String condition) {
        Connection conn = null;
        try {
            conn = databaseConnectionPool.getConnection();
            DSLContext create = DSL.using(conn, SQLDialect.POSTGRES);
            if (condition != null) {
                PreparedStatement st = conn.prepareStatement("DELETE FROM " + tableName + " WHERE " + columnName + " = ?");
